...
export class Level {
    ...
    static async load(name: string): Promise<Level | undefined> {
        ...
        try {
            ...
            if (magic !== MCS_MAGIC_NUMBER) throw new Error('Not a valid MCS level file')
            ...
            switch (version) {
                case 1: {
                    ...
                    level = new Level(name, width, height, depth, { x: spawnX, y: spawnY, z: spawnZ, rotx: spawnYaw, roty: spawnPitch }, blocks)
                    break
                }
                default:
    ...
    static async create({ name, width, height, depth, spawn }: LevelCreateOptions): Promise<Level> {
        // TODO: If a level with this name already exists, just return it with a warning.
        // TODO: Separate out level generation

        const blocks: number[] = new Array(width * height * depth).fill(Block.AIR.id)
